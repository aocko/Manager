...
public class Timetable2Dao {
    public List<Timetable> timetable2List(Connection con, String classId) throws SQLException {
        String sql = "select * from t_timetable2 where classId=?";
    ...
    public int timetable2ListUpdate(Connection con,String classId, List<Timetable> timetableList) {
       timetableList.forEach(timetable -> {
           try {
               timetbale2Update(con, classId, timetable);
    ...
    public int timetbale2Update(Connection con, String classId, Timetable timetable) throws SQLException {
        String sql = "update t_timetable2 set first=?,second=?,third=?,fourth=?,fifth=?,sixth=?,seventh=?,eighth=?,ninth=?,tenth=?,eleventh=?,twelfth=?,thirteenth=? where classId=? and week=?";
        PreparedStatement pstmt = con.prepareStatement(sql);
    ...
    public int saveTimetable2List(Connection con,String classId, List<Timetable> timetableList) {
        timetableList.forEach(timetable -> {
            try {
                saveTimetable2(con, classId, timetable);
            } catch (SQLException e) {
                e.printStackTrace();
            }
        });
  ...
  public int saveTimetable2(Connection con,String classId,Timetable timetable) throws SQLException {
      String sql = "insert into t_timetable2 values (null ,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
      PreparedStatement pstmt = con.prepareStatement(sql);
      pstmt.setString(1, classId);
